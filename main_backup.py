...
def build_faiss_index(targets: list) -> faiss.Index:
    """
    targets: list of tuples (embedding, name)
    return: faiss index, list of names in same order
    """
    embeddings = np.array([t[0] for t in targets]).astype("float32")
    # Normalize embeddings để cosine similarity = inner product
...
def frame_processor(
    ...
    for bbox, kps in zip(bboxes, kpss):
        ...
        if best_match_name != "Unknown":
            ...
            logger.log(best_match_name)   # NEW: log attendance
        else:
...
def main(params):
    ...
    logger = AttendanceLogger("database/attendance.db")   # NEW
